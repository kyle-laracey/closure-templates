    ...
    return entryPoint(
        () -> {
          ...
          for (SoyFileNode file : files.getChildren()) {
            ...
            output.append("\n// Import the generated Sass header files for all dependencies\n");
            file.getImports().stream()
                ...
                .sorted()
                .forEach(
                    soyImport -> output.append("@import('").append(soyImport).append(".sass')\n"));

            output.append("\n// Import the user-written CSS files required\n");
            file.getRequireCss().stream()
                .distinct()
                .sorted()
                .forEach(
                    sassImport ->
